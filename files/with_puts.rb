...
# In this example, we open the file for reading and updating with a+ mode, declare a variable file as a placeholder for thefile and append more text to it with the shovel operator. Note that the file is automatically closed after the operation (the end of the block) and see below, we can use puts or write in place of <<.
...
1: Here we are with a new line of text, Writing to files in Ruby is simple.
